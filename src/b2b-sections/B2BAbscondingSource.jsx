...
function RollingAmountTable({ title, accentClass, months, section }) {
              ...
              >
                ...
                {months.map((m) => (
                  <td key={m} className="px-4 py-2.5 text-right tabular-nums">
                    {fmtCurrency(row.monthly_amounts?.[m] ?? 0)}
                  </td>
                ))}
              </tr>
            ))}
          ...
          {section.total && (
            <tfoot>
              <tr className="border-t-2 border-slate-200 bg-slate-50 font-bold text-slate-900">
                ...
                {months.map((m) => (
                  <td key={m} className="px-4 py-2.5 text-right tabular-nums">
                    {fmtCurrency(section.total.monthly_totals?.[m] ?? 0)}
                  </td>
                ))}
              </tr>
            </tfoot>
...
export default function B2BAbscondingSource({ data }) {
  if (!data?.months) return null;

  return (
    <div className="space-y-0 divide-y divide-slate-100">
      <RollingAmountTable
        title="Absconding Sold"
        accentClass="bg-rose-50/60 text-rose-700"
        months={data.months}
        section={data.absconding_sold}
      />
      <RollingAmountTable
