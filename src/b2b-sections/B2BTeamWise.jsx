...
export default function B2BTeamWise({ data }) {
        ...
        <table key={team.sales_team} className="w-full text-sm mb-0">
          <thead>
            ...
            <tr className="bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500">
              ...
              <th className="px-4 py-2 text-left border-b border-slate-200 whitespace-nowrap">Staff</th>
              {months.map((m, i) => (
              ...
              >
                ...
                {months.map((m) => (
                  <td key={m} className="px-4 py-2.5 text-right tabular-nums">
                    {fmtInt(staff.monthly_counts?.[m] ?? 0)}
                  </td>
                ))}
              </tr>
            ))}
          ...
          {team.team_totals && (
            <tfoot>
              <tr className="border-t-2 border-slate-200 bg-slate-50 font-bold text-slate-900">
                ...
                <td className="px-4 py-2.5 font-extrabold whitespace-nowrap">Team Total</td>
                {months.map((m) => (
                  <td key={m} className="px-4 py-2.5 text-right tabular-nums">
                    {fmtInt(team.team_totals.monthly_totals?.[m] ?? 0)}
                  </td>
                ))}
              </tr>
            </tfoot>
