...
// ── Absconding Details table ─────────────────────────────────────────────────
function AbscondingDetails({ details }) {
          ...
          <tfoot>
            <tr className="border-t-2 border-slate-200 bg-slate-50 font-bold text-slate-900">
              ...
              <td className="px-4 py-2.5 text-right tabular-nums">
                {fmtInt(details.totals.total_count)}
              </td>
              <td className="px-4 py-2.5 text-right tabular-nums">
                {fmtCurrency(details.totals.total_amount)}
              </td>
              <td className="px-4 py-2.5 text-right tabular-nums text-emerald-600">
                {fmtCurrency(details.totals.total_received)}
              </td>
              <td className="px-4 py-2.5 text-right tabular-nums text-rose-500">
                {fmtCurrency(details.totals.total_balance)}
              </td>
            </tr>
          </tfoot>
        )}
...
// ── Monthly Comparison rolling table ─────────────────────────────────────────
function MonthlyBlock({ title, accent, months, section }) {
  if (!section) return null;

  return (
    <div>
      <div className={`px-4 py-2 border-y border-slate-100 ${accent} text-xs font-bold uppercase tracking-wider`}>
        {title}
      </div>
      <div className="overflow-x-auto">
...
export default function BranchesAbsconding({ details, monthly }) {
  return (
      ...
      <div>
        <div className="px-4 py-2 bg-slate-100 border-b border-slate-200">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-700">
            Absconding Customer Details
          </span>
        </div>
        <AbscondingDetails details={details} />
      </div>
